    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden gradient-hero items-center justify-center p-12">
        <div className="absolute inset-0">
          <div className="absolute top-1/3 right-1/4 w-96 h-96 bg-purple/10 rounded-full blur-[120px] animate-pulse-glow" />
          <div className="absolute bottom-1/4 left-1/3 w-80 h-80 bg-indigo/10 rounded-full blur-[100px] animate-pulse-glow" style={{ animationDelay: "2s" }} />
        </div>
        <div className="relative z-10 max-w-md">
          ...
          >
            Join thousands of engineers who transformed their careers with AI-powered practice.
          </motion.p>
        </div>
      </div>
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <Input id="name" placeholder="John Doe" value={name} onChange={(e) => setName(e.target.value)}
                  className="pl-10 h-11 rounded-xl bg-muted/50 border-border" required />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input id="password" type={showPassword ? "text" : "password"} placeholder="Min 8 characters" value={password}
                  onChange={(e) => setPassword(e.target.value)} className="pl-10 pr-10 h-11 rounded-xl bg-muted/50 border-border" required />
            ...
            <button type="submit" disabled={isLoading}
              ...
              {isLoading ? <div className="h-4 w-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <>Create Account <ArrowRight className="h-4 w-4" /></>}
            </button>
          </form>

          <p className="text-center text-sm text-muted-foreground mt-6">
            Already have an account?{" "}
            <Link href="/login" className="text-indigo-light hover:text-indigo font-medium transition-colors">Log in</Link>
          </p>
        </motion.div>
      </div>
    </div>
